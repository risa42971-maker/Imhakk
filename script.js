...
function loadToolContent(tool) {
    ...
    switch(tool) {
        ...
        case 'qrcode':
            ...
            break;
        case 'binary':
            container.innerHTML = getBinaryToolHTML();
            break;
        case 'useragent':
...
// ==================== NEW: BINARY TOOL HTML ====================
function getBinaryToolHTML() {
    return `
        <div class="tool-header">
            <div class="tool-icon">🤖</div>
            <div class="tool-title">
                <h2>BINARY CONVERTER</h2>
                <p>>_ Convert Text ↔ Binary (0s and 1s)</p>
            </div>
        </div>
        
        <div class="input-group">
            <label><i class="fas fa-font"></i> TEXT TO BINARY :</label>
            <textarea id="binaryTextInput" class="input-field" rows="3" placeholder="Enter text to convert to binary..."></textarea>
        </div>
        
        <button class="btn-primary" onclick="textToBinary()" style="margin-bottom: 15px;">
            <i class="fas fa-arrow-right"></i> TEXT → BINARY
        </button>
        
        <div class="input-group">
            <label><i class="fas fa-robot"></i> BINARY TO TEXT :</label>
            <textarea id="binaryInput" class="input-field" rows="3" placeholder="Enter binary (e.g., 01001000 01101001)"></textarea>
        </div>
        
        <button class="btn-primary" onclick="binaryToText()">
            <i class="fas fa-arrow-left"></i> BINARY → TEXT
        </button>
        
        <div id="binaryResult" class="result-box" style="display: none;">
            <div class="result-title">
                <i class="fas fa-file-alt"></i> RESULT:
                <button class="btn-secondary" style="margin-left: auto; padding: 5px 10px;" onclick="copyBinaryResult()">
                    <i class="fas fa-copy"></i> COPY
                </button>
            </div>
            <div id="binaryContent" class="result-content" style="word-break: break-all; font-family: monospace;"></div>
        </div>
        
        <div id="binaryError" class="error-message" style="display: none;"></div>
